...
<body>
    ...
    <div class="screen">
        ...
        <label class="ineer_one">欢迎您的使用</label><br/>
        <label class="ineer_two">电影订票系统</label>
    </div>
    <!-- 登录页 -->
    <div class="content" style="float: left;">
        ...
        <div class="page">
            ...
            <div class="childpage" style="display: block">
                ...
                <div>
                    <label class="login_version">@电影订票</label>
                </div>
            </div>
            <!-- 注册界面 -->
            <div class="childpage" style="display: none">
                ...
                <div>
                    <label class="register_version">@电影订票</label>
                </div>
            </div>
        </div>
    ...
    　　};
        ...
        function registerbtn(){
            ...
            var patrn=/^([a-zA-Z0-9]){6,11}$/;
            if (!patrn.exec(user_name)){
                register_error.text("账号只能输入6-11个字符串");
                textNum.value = TestNum();
                return;
            }
            var patrnPwd=/^([a-zA-Z0-9]){6,11}$/;
            if (!patrnPwd.exec(user_pwd)){
                register_error.text("密码只能输入6-11字符串");
                textNum.value = TestNum();
                return;
            }
            var patrnEml=/^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/;
            if (!patrnEml.exec(user_email)){
                register_error.text("请输入正确的邮箱");
                textNum.value = TestNum();
                return;
            }
